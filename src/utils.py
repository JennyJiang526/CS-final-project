...
class DocExtracter:
    ...
    def extract(self, ids):
        # ============================================================
        # [修改] extract 方法完整重写，两个分支都加了缺失 ID 的安全跳过逻辑
        # 原始代码直接用 self.dict[key] 取值，遇到 index 有但 corpus 没有
        # 的 ID（版本不对齐时常见）会直接 KeyError 崩溃。
        # 现在改为：先提取 key，检查存在性，不存在则 skip + 打印警告。
        # ============================================================
        if self.cache:
            output = []
            for i in ids:
                ...
                # 改为先取 key，再做存在性检查
                key = i if type(i) == str else i["id"]
                if key not in self.dict:
                    # [修改] 新增：跳过缺失 ID，打印警告而不是崩溃
                    print(f"[Warning] ID '{key}' not found in cache dict, skipping.")
                    continue
                item = self.dict[key]
                output.append(item)
        else:
            output = []
            for i in ids:
                # [修改] 同上，else 分支也加了相同的安全检查
                # 原始代码：item = self.dict[i] if type(i) == str else self.dict[i["id"]]
                key = i if type(i) == str else i["id"]
                if key not in self.dict:
                    # [修改] 新增：跳过缺失 ID，打印警告而不是崩溃
                    print(f"[Warning] ID '{key}' not found in path dict, skipping.")
                    continue
                item = self.dict[key]
