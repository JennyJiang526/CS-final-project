...
from adaptive_k_llm import AdaptiveKLLM
from adaptive_k_classifier import AdaptiveKClassifier
...
class MedRAG:
    ...
    def __init__(self, llm_name="OpenAI/gpt-3.5-turbo-16k", rag=True, follow_up=False, retriever_name="MedCPT", corpus_name="Textbooks", db_dir="./corpus", cache_dir=None, corpus_cache=False, HNSW=False, 
                 k=32, adaptive_k=False, use_llm_k=False):
        ...
        if adaptive_k:
            if use_llm_k:
                self.k_predictor = AdaptiveKLLM(self)
            else:
                self.k_predictor = AdaptiveKClassifier("classifier.pkl")
        ################################################################
    ...
    def medrag_answer(self, question, options=None, k=32, rrf_k=100, save_dir = None, snippets=None, snippets_ids=None, format_instruction="", **kwargs):
        '''
        ...
        if self.rag:
            if snippets is not None:
                ...
                if self.adaptive_k:
                    k = self.k_predictor.predict_k(question)
        ...
        if not self.rag:
            ...
            for context in contexts:
                prompt_medrag = self.templates["medrag_prompt"].render(context=context, question=question, options=options, format_instruction=format_instruction)
                messages=[
